(function () {
  ...
  function play(kind, duration) { var list = frames[kind] || frames.idle, i=0; clearInterval(frameTimer); cat.src = root + list[0]; frameTimer = setInterval(function(){ i=(i+1)%list.length; cat.src=root+list[i]; }, kind==='sleep'?1200:260); setTimeout(function(){ clearInterval(frameTimer); cat.src=root+list[0]; }, duration || 2200); }
  ...
  window.jinzhuOverlay.getState().then(function(s){ state=Object.assign(state,s||{}); play('idle',999999); if(state.fullness<25)say('个饭碗好似空咗喔。'); });
